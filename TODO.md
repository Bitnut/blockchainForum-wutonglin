...
开发过程中，用到的许多框架中其中比较让我不满意的就是 antd 这个 UI 框架了，这个框架的可定制性真的很低，而且学习成本蛮高的。另外运行的速度较慢和代码封装度高导致的定制性低是比较让我头疼的地方。用了他的素材库导致网页打包特别大，结果单元测试的时候覆盖率测不出来。。。我枯了。不过从美观性上来讲 antd 还蛮不错的(其实都能做得到反而不算优点了：D)。因此重构代码的工作会包含更换 UI 框架。
